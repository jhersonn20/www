...
class RUSER_Model extends CI_Model {
    ...
    public function getUser($filters){
		...
		if ($query->num_rows() > 0){
			...
			if ($query->num_rows() > 0)
				return array("user_id"=>$query->row()->user_id,"id"=>$query->row()->id,"first_name"=>$query->row()->first_name,"last_name"=>$query->row()->last_name,"expiry"=>$query->row()->expiry,"client_id"=>$query->row()->client_id,"creator"=>$query->row()->creator,"department"=>$query->row()->department);
			else
				return false;
		}else
			return false;            
