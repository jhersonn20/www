<?php
    class Tfab_Mat_Spl_Model extends CI_Model {
    	private $tblName = "tfab_mat_spl";
		...
		public function get_all_query($where){
        	//$this->call_sp_query_spl_wise((int) $_GET['tbAdvance'], $_GET['rswork'],$_GET['aname'],$area);
			$start = ($_GET['page'] - 1) * $_GET['pageSize'];
			$end = ($start + $_GET['pageSize']) - 1;
		...
		public function export_tfab_mat_spl($where = ""){
			if ($where != "")
				$where = ("WHERE " . $where);
		...
		public function call_upl_tfab(){
			$sql = "declare @result int = 0;";			
			$sql .= "exec @result = qms_pip.dbo.upl_tfab_mat_spl @ip_filename = 'fabshopUPL';";
