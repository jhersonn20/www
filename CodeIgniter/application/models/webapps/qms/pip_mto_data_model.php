<?php
    class Pip_mto_data_Model extends CI_Model {
    	private $tblName = "pip_mto_data";
        ...
        public function set_pip($rowArr = array()){
			$postInfo = array();
			if (sizeof($rowArr) > 0)
				$postInfo = $rowArr;
			else {
		...
		public function call_sp($item = ""){
			...
			if ($item == "")
				$sql .= "exec @result = qms_pip.dbo.pipingMTO_sp;";
			else if ($item == 'mto1')
				$sql .= "exec @result = qms_pip.dbo.isoMQ1_sp;";
			else if ($item == 'mto2')
				$sql .= "exec @result = qms_pip.dbo.isoMQ2_sp;";
			else if ($item == 'weld1')
				$sql .= "exec @result = qms_pip.dbo.isoWQ1_sp;";
			else if ($item == 'weld2')
				$sql .= "exec @result = qms_pip.dbo.isoWQ2_sp;";
			else if ($item == 'ps1')
				$sql .= "exec @result = qms_pip.dbo.isoPS1_sp;";
			else if ($item == 'wends')
				$sql .= "exec @result = qms_pip.dbo.isoWends_sp;";
			else if ($item == 'whseSpl')
				$sql .= "exec @result = qms_pip.dbo.isoWhseSpl_sp;";
			else if ($item == 'fabSpl')
				$sql .= "exec @result = qms_pip.dbo.isoFabSpl_sp;";
			else if ($item == 'pwEngg')
				$sql .= "exec @result = qms_pip.dbo.pwEngg_sp;";
			else if ($item == 'pwQaqc')
				$sql .= "exec @result = qms_pip.dbo.pwQaqc_sp;";
			else if ($item == 'pwWhse')
				$sql .= "exec @result = qms_pip.dbo.pwWhse_sp;";
			else if ($item == 'pwFab')
				$sql .= "exec @result = qms_pip.dbo.pwFab_sp;";
			else if ($item == 'pmWm')
				$sql .= "exec @result = qms_pip.dbo.pmWm_sp;";
			else
				$sql .= "exec @result = qms_pip.dbo.{$item}_sp;";
