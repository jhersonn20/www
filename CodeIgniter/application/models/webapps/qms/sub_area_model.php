<?php
    class Sub_area_Model extends CI_Model {
        ...
        public function set(){		
			...
			$postInfo['log_update'] = $_POST['log_user'] . " " . mdate("%H:%i:%s");
			$postInfo['log_date'] = mdate("%H:%i:%s");

            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('subarea_code' => $_POST['subarea_code']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'subarea_code' found for '{$_POST['subarea_code']}' item!";
	            		return $query;
	            	}
            	}else
            		return $query;
