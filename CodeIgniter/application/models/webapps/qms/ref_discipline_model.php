<?php
    class Ref_discipline_Model extends CI_Model {
    	private $tblName = "ref_discipline";
        ...
        public function set(){		
            ...
            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('discipline_code' => $_POST['discipline_code']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'discipline_code' found for '{$_POST['discipline_code']}' item!";
	            		return $query;
	            	}
            	}else
            		return $query;
