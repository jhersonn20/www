<?php
    class Sub_System_Model extends CI_Model {
    	private $tblName = "sub_system";
		...
		public function get_all_dd2(){
			$sql = "select t1.system_no,sub_system from {$this->tblName} t1 join (SELECT RTRIM(LTRIM(system_no)) as system_no from {$this->tblName} where RTRIM(LTRIM(system_no)) <> '' group by RTRIM(LTRIM(system_no))) t2 on t1.system_no = t2.system_no";
