<?php
    class Action_Model extends CI_Model {
		...
		public function get_all_dd(){
			$this->piping->select('action_code, action_desc');
        ...
        public function set(){		
            ...
            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('action_code' => $_POST['action_code']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'action_code' found for '{$_POST['action_code']}' item!";
	            		return $query;
	            	}
            	}else
            		return $query;
