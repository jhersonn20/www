<?php
    class Ref_jointtype_Model extends CI_Model {
    	private $tblName = "ref_jointtype";
        ...
        public function set(){		
            ...
            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('joint_type' => $_POST['joint_type']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'joint_type' found for '{$_POST['joint_type']}' item!";
	            		return $query;
	            	}
            	}else
            		return $query;
