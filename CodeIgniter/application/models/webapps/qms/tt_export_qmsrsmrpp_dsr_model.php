<?php
    class Tt_Export_Qmsrsmrpp_Dsr_Model extends CI_Model {
    	private $tblName = "tt_export_qmsrsmrpp_dsr";
		...
		public function export_structDR($where){
		...
		public function call_sp_query_export_structDR($startDate,$endDate,$priorno,$areano,$drawingno,$jmifno,$rsOption){
			$sql = "declare @result int;";
			$sql .= "exec @result = piping.dbo.export_structDailyReport @cStartD = '{$startDate}',@cEndD ='{$endDate}',@cPriorno = '{$priorno}',@cAreano = '{$areano}',@cDrawingno = '{$drawingno}',@cJmifno = '{$jmifno}',@rsOption = {$rsOption};";
			$sql .= "select @result as return_value;";
