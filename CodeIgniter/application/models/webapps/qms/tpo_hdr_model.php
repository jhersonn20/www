<?php
    class Tpo_hdr_model extends CI_Model {
    	private $tblName = "tpo_hdr";
        ...
        public function get_all($where){
		 ...
		 public function get_all_jmif($where){
			$start = ($_GET['page'] - 1) * $_GET['pageSize'];
			$end = ($start + $_GET['pageSize']) - 1;
        ...
        public function set(){
            ...
            if ($_POST['PROGRESS_RECID'] > 0){
	            ...
	            $control_no = $ci->rcontrol_model->update_control("inc", array("trancode" => "PO"));
		...
		public function remove(){
			...
			// $query = $this->piping->get();
			$post_control_no = intval(substr($_POST['po_no'], 3,6));
			
			if ($post_control_no != ($control_no[0]['control_no'] - 1))
				return "Cannot Delete.\nA higher PO No. exist.";
			
	        $control_no = $ci->rcontrol_model->update_control("dec", array("trancode" => "PO"));
        	$query = $this->piping->where('PROGRESS_RECID', $_POST['PROGRESS_RECID']);
