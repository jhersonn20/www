<?php
    class Twhse_mat_spl_Model extends CI_Model {
    	private $tblName = "twhse_mat_spl";
		...
		public function mod_export_psmto_twhseMatSpl($where = ""){
			$sql = "SELECT TOP 100 (ISNULL(t.drawing_no,'')+ '-' + ISNULL(t.spool_no,'')) as stock_no,t.spool_no,t.plant_no,t.area_no,t.drawing_no,t.sheet_no,t.bore_type,t.matl_type,t.piping_class,t.priority,t.area_loc,
	                (CASE WHEN(t.ref_date IS NULL) THEN('') ELSE(convert(varchar, t.ref_date, 101)) END) as ref_date,t.client_ref_no,(CASE WHEN(t.ref_date IS NULL) THEN(convert(varchar, t.client_ref_date, 101)) ELSE('') END) as client_ref_date,t.whse_bin,t.remarks
					FROM dbo.twhse_mat_spl t
					LEFT JOIN(
				    ...
				    t.spool_no = t4.spool_no
					UNION ALL
					SELECT (ISNULL(t.drawing_no,'')+ '-' + ISNULL(t.spool_no,'')) as stock_no,t.spool_no,t2.plant_no,t2.area_no,t2.drawing_no,t2.sheet_no,t.lbsb as bore_type,t.mat_type,t2.piping_class,t2.priority_timing as priority,t2.area_loc,'' as ref_date,''as client_ref_no,''as client_ref_date,'' as whse_bin,'' as remarks
					...
					WHERE t.spool_no <> 'EM' AND
					      t2.priority_timing in (
													SELECT p_char2 
        ...
        public function get_all_export2($where = ""){
			...
			$sql = "select TOP 100 t.drawing_no + '-' + t.spool_no,
						   t.spool_no,t2.plant_no,t2.area_no,
						   t2.drawing_no,t2.sheet_no,t.lbsb,
						   t.mat_type,t2.piping_class,t2.priority_code,
					from piping.dbo.spool t
					inner join piping.dbo.iso t2
						on t2.plant_no = t.plant_no and
						   t2.area_no = t.area_no and
						   t2.drawing_no = t.drawing_no and
						   t2.sheet_no = t.sheet_no";
			$rowArr = $this->qms_pip->query($sql)->result_array();
			return $rowArr;
        }
