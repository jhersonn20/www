<?php
    class Iso_mech_Model extends CI_Model {
    	private $tblName = "iso_mech";
		...
		public function call_sp_isoEquip($file){
			
			$sql = "declare @result int = 0;";			
			$sql .= "exec @result = piping.dbo.upl_iso_equipment @ip_filename =" ."'" .str_replace(".csv", "", $file). "'". ";";
			$sql .= "select @result as return_value;";
			return $this->piping->query($sql)->result_array();
		}
		public function remove_isoMech(){
        	$query = $this->piping->where('PROGRESS_RECID', $_POST['PROGRESS_RECID']);
