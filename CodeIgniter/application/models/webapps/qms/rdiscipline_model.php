<?php
    class Rdiscipline_Model extends CI_Model {
    	private $tblName = "rdiscipline";
        ...
        public function get_by_field($fields){
			...
			$sql = "SELECT disc_desc from {$this->tblName} where disc_code = ?";
        ...
        public function set(){
			...
			$postInfo['log_date'] = mdate("%Y-%m-%d");

            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('disc_code' => $_POST['disc_code']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'disc_code' found for '{$_POST['disc_code']}' item!";
	            		return $query;
	            	}
            	}else
            		return $query;
