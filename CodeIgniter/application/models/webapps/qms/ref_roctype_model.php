<?php
    class Ref_roctype_Model extends CI_Model {
    	private $tblName = "ref_roctype";
		...
		public function get_all_dd(){
			$this->piping->select('roc_type, type_desc');
        ...
        public function set(){		
            ...
            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('roc_type' => $_POST['roc_type'], 'discipline_code' => $_POST['discipline_code']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate 'roc_type' found for '{$_POST['roc_type']}' item in '{$_POST['discipline_code']}' discipline!";
	            		return $query;
	            	}
            	}else
            		return $query;
