<?php
    class Elec_takeoff_Model extends CI_Model {
    	private $tblName = "elec_takeoff";
		...
		public function call_upl_elec($file){
			
			$sql = "declare @result int = 0;";			
			$sql .= "exec @result = piping.dbo.upl_elecTakeoff @ip_filename =" ."'" .str_replace(".csv", "", $file). "'". ";";
			$sql .= "select @result as return_value;";
			return $this->piping->query($sql)->result_array();
		}
		public function remove_elec(){
        	$query = $this->piping->where('PROGRESS_RECID', $_POST['PROGRESS_RECID']);
