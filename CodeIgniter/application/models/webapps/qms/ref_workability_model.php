<?php
    class Ref_workability_Model extends CI_Model {
    	private $tblName = "ref_workability";
        ...
        public function set(){		
            ...
            if ($_POST['PROGRESS_RECID'] == "0"){
            	$query = $this->is_entry_unique(array('year' => $_POST['year'], 'month' => $_POST['month'], 'week' => $_POST['week']));
            	if (gettype($query) == 'boolean'){
            		if (!$query){
	            		$query = "Failed! Duplicate record found for '{$_POST['year']}', '{$_POST['month']}', '{$_POST['week']}'!";
	            		return $query;
	            	}
            	}else
            		return $query;
