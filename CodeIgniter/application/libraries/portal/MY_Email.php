<?php
	...
	class MY_Email extends CI_Email {
		private $msgHeader = "You may now log-in to the <a href='https://portal.arcc-eei.com/' style='text-decoration: none;color: #0f73a9;'> ARCC Portal </a> using the information below:";
		private $msgFooter = "If you're not expecting any files from ARCC, kindly ignore this email. <br />Please do not reply to this mail as it is automated and cannot be responded to. This email was sent by Al Rushaid Construction Company, Limited.";
		private $msgFooter2 = "Please do not reply to this mail as it is automated and cannot be responded to. This email was sent by Al Rushaid Construction Company, Limited.";
		private $body = "";		
		...
		private function chnge_password($params = array()){
			$this->subject("ARCC Portal: Password Change Successful");
			$this->message($this->build_message('pcs', $params));
			$this->set_mailtype("html");			
			return $this->before_sending('Login Credential');
		...
		private function upload(){
			$this->msgHeader = "File has been uploaded in ARCC Portal dedicated exclusively for your good company. Here is the <a href='https://portal.arcc-eei.com/' style='text-decoration: none;color: #0f73a9;'> link </a> that will directed you to the login page of the portal.";
		...
		private function upload_by_client($params = array()){
			...
			$this->msgHeader = "File has been uploaded by {$params['user_name']} of {$params['client_name']}.";
		...
		private function download_by_client($params = array()){
			...
			$this->msgHeader = "File has been downloaded by {$params['user_name']} of {$params['client_name']}.";
			$this->msgFooter = $this->msgFooter2;
			$this->msgFooter = $this->msgFooter2;
			$this->subject("ARCC Portal: File Review");
		...
		private function build_message($type = '', $params = array()){
			...
			switch ($type) {
				...
				case 'pcs':
					$this->body .= "<ul>";
					$this->body .= "<li> <label>User ID:</label> {$params['user_id']} </li>";
					$this->body .= "<li> <label>Password:</label> {$params['newpass']} </li>";
					//$this->body .= "<li> <label>Expiration:</label> " . date_format(new DateTime($params['expiry']), 'g:ia \o\n l jS F Y') . " </li>";
					$this->body .= "</ul>";
					break;					
				...
				case 'upload_by_client':
					$this->body .= "<ul>";
					if ($params['remarks'] != "")
						$this->body .= "<li> <label>Remarks:</label> {$params['remarks']} </li>";
