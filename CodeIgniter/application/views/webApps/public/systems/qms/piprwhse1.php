...
<div id="main-wrapper">
	<div class="wrap-button demo-section" style="width: 39.5%;min-height: 530px;position: fixed;top: 120px;left: 400px;">
		<fieldset style="width: 500px;">
		
		<div>
		<label class="title" for="sel1" >Spool Type:</label><select name="sel1" id="sel1" style="width: 300px" ></select>
		</div>
		<div><br /></div>
		<div>
			<label class="title" for="option" style="width: 100px;">ALL</label><input type="radio" name="option" checked id="rad1" value="ALL" />
        	<label class="title" for="option" style="width: 100px;">LB</label><input type="radio" name="option" id="rad2" value="LB" />
        	<label class="title" for="option" style="width: 100px;">SB</label><input type="radio" name="option" id="rad3" value="SB" />
		</div>
		<div><br /></div>
		<div>
		...
		<div><br /></div>
		<div>
			<label class="title" for="sel3">System No:</label><select name="sel3" id="sel3" style="width: 300px;"></select>
		</div>
		<div><br /></div>
		<div>
			<label class="title" for="sel4">SubSystem No:</label><select name="sel4" id="sel4" style="width: 300px;"></select>
		</div>
		<div><br /></div>
		<div>
			<label class="title" for="sel5" style="width: 100px;">TestPackage #:</label>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
			...
			var spoolType = $("#sel1").kendoMultiSelect({
            filter: "contains",
            placeholder: "Select Spool type...",
            dataTextField: "spltype_code",
            dataValueField: "spltype_code",
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/spltype_ref",
                    	type: "GET"
				    }
                },
        ...
        var areaNo = $("#sel2").kendoMultiSelect({
            ...
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/areano_ref",
                    	type: "GET"
				    }
                },
                //serverFiltering: true,
        ...
        var systemNo = $("#sel3").kendoMultiSelect({
            filter: "contains",
            placeholder: "Select System No...",
            dataTextField: "system_no",
            dataValueField: "system_no",
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/subsystemNo_ref",
                    	type: "GET"
				    }
                },
                //serverFiltering: true,
        ...
        var subSystem = $("#sel4").kendoMultiSelect({
            ...
            dataTextField: "sub_system",
            dataValueField: "sub_system",
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/subsystemNo_ref",
		...
		$("#openWindow").click(function(e){
			...
			$(".wrap-button .buttonRight button").bind({
		    	click: function(e){
		    		switch(this.id){
		    			case "printButt":
		    				//alert(areaNo.value() + " , " + $('input[name=option]:checked').val() + " , " + spoolType.value() + " , " + subSystem.value() + " , " + systemNo.value() + " , " + kendo.toString(start.value(),"yyyy-MM-dd"));
	    				if (confirm("Do you really want to print this item?")){	    					
							...
							$("#window").data("kendoWindow").refresh({
							    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/SPOOL AT WAREHOUSE RECEIVED&cArea=" + areaNo.value() + "&cBore=" + $('input[name=option]:checked').val() + "&cSpltype=" + spoolType.value() + "&cSubSys=" + subSystem.value() + "&cSystem=" + systemNo.value() + "&cTP=" + $("#textarea").val() + "&dtAsof=" + kendo.toString(start.value(),"yyyy-MM-dd") + "&program_name2=" + "piprwhse1.php",
