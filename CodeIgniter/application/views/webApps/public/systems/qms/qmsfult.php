<div id="main-wrapper">
	<div style="min-height: 198px;margin-bottom: 5px;">
		<div class="wrap-form demo-section apply8" style="width: 37.1%;float:right;height: auto;display: block;">
			<fieldset>
				<legend> Material Utilization Reference: </legend>
				<ul class="formLeft_qms" style="width: 100%;">
					<li>
						<label class="title" for="txt1" style="width: 130px;">Utility Code:</label><input type="text" required name="txt1" id="txt1" class="k-textbox" style="width: 61%;" />
					</li>
					<li>
...
<script type="text/javascript">
	function grid_change(e){
	    ...
	    for (var i = 0; i < selectedRows.length; i++) {
	      dataItem = e.dataItem(selectedRows[i]);
		    $("#txt1").val(dataItem.util_code);
		    $("#textarea").val(dataItem.util_desc);
	...
	$(document).ready(function(){
        ...
        var dataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: crudService + "directCall/matUtil",
                    contentType: "application/json",
                    type: "GET"
                },
                create: {
                    url: crudService + "manage/matUtil",
                    type: "POST",
                ...
                destroy: {
                    url: crudService + "remove/matUtil",
                    type: "POST"
                },
			    parameterMap: function(data, type) {
			      if (type == "read") {
				    ...
				    fieldSort = ($(data.sort).length ? data.sort[0].field : "util_code");
				    query = filterFArr;
        ...
        var grid = $("#rowSelection").kendoGrid({
            ...
            columns: [
               {field: "util_code",title: "Utility Code",width: 107},
               {field: "util_desc",title: "Description"}
           ],
           change: function(e){
           		currRow = this;
        ...
        insertGridTitle("#rowSelection","Material Utilization Listing");
	    		...
	    		switch(this.id){
	    			case "saveButt":
						...
						if (cMode == "add"){
					        ...
					        $.post(crudService + "manage/matUtil",{PROGRESS_RECID: dataItem.PROGRESS_RECID,util_code: $("#txt1").val(), util_desc: $("#textarea").val(), flg_status: ($("#rad1").is(":checked") ? 1 : 0)},
					       	    function(data){
									$("#rowSelection").data("kendoGrid").setDataSource(dataSource);
									$("#rowSelection").data("kendoGrid").dataSource.page($("#rowSelection").data("kendoGrid").dataSource.page());
									$("#rowSelection").data("kendoGrid").dataSource.read();
