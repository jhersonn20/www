<div id="main-wrapper" style="min-height: 1000px">
	<div class="wrap-button demo-section" style="width: 200px;min-height: 220px;">
		<fieldset style="min-height: 205px;">
			<legend> Piping Workability: </legend>
            <div style="float: left;line-height: 25px;margin-bottom: 10px">
            	<fieldset>
            	<label><input type="radio" name="option" checked id="option1" /> Per Piecemark </label>
		    				...
		    				if($('input[name=option]:checked').index('input[name=option]') == 0){
		    					if(($("#chk1").is(":checked") ? 1 : 0) == 0){
									...
									$("#window").data("kendoWindow").refresh({
									    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/QMS WORKABILITY PER STRL PIECE" +"progname="+"Per Spool" +"&rsOption=" + $('input[name=option]:checked').index('input[name=option]') +"&rsOption2=" + $('input[name=option2]:checked').index('input[name=option2]') +"&tg1=" + ($("#chk1").is(":checked") ? 1 : 0), 
									...
									$("#window").data("kendoWindow").refresh({
									    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/QMS WORKABILITY PER STRL PIECE DETAILED" +"progname="+"Per Spool" +"&rsOption=" + $('input[name=option]:checked').index('input[name=option]') +"&rsOption2=" + $('input[name=option2]:checked').index('input[name=option2]') +"&tg1=" + ($("#chk1").is(":checked") ? 1 : 0), 
									    contentType: "application/pdf"
									});
							        $("#window").data("kendoWindow").center().open();
							       }
		    					...
		    					if(($("#chk1").is(":checked") ? 1 : 0) == 0){
									...
									$("#window").data("kendoWindow").refresh({
									    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/QMS WORKABILITY PER STRL ISO" +"progname="+"Per Spool" +"&rsOption=" + $('input[name=option]:checked').index('input[name=option]') +"&rsOption2=" + $('input[name=option2]:checked').index('input[name=option2]') +"&tg1=" + ($("#chk1").is(":checked") ? 1 : 0), 
									    contentType: "application/pdf"
									});
							        $("#window").data("kendoWindow").center().open();
							        }
									...
									$("#window").data("kendoWindow").refresh({
									    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/QMS WORKABILITY PER STRL PIECE ISO DETAILED" +"progname="+"Per Spool" +"&rsOption=" + $('input[name=option]:checked').index('input[name=option]') +"&rsOption2=" + $('input[name=option2]:checked').index('input[name=option2]') +"&tg1=" + ($("#chk1").is(":checked") ? 1 : 0), 
									    contentType: "application/pdf"
									});
							        $("#window").data("kendoWindow").center().open();
							       }
