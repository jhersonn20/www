<div id="main-wrapper">
	<div style="min-height: 198px;margin-bottom: 5px;">
		<div class="wrap-form demo-section apply8" style="width: 37.1%;float:right;height: auto;display: block;">
			<fieldset>
				<legend> Reason Reference: </legend>
				<ul class="formLeft_qms" style="width: 100%;">
					<li>
						<label class="title" for="txt1" style="width: 130px;">Reason Type:</label><input type="text" required name="txt1" id="txt1" class="k-textbox" style="width: 61%;" />
					</li>
					<li>
...
<script type="text/javascript">
	function grid_change(e){
	    ...
	    for (var i = 0; i < selectedRows.length; i++) {
	      dataItem = e.dataItem(selectedRows[i]);
		    $("#txt1").val(dataItem.reason_code);
		    $("#textarea").val(dataItem.reason_desc);
	...
	$(document).ready(function(){
        ...
        var dataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: crudService + "directCall/reason",
                    contentType: "application/json",
                    type: "GET"
                },
                create: {
                    url: crudService + "manage/reason",
                    type: "POST",
        ...
        var grid = $("#rowSelection").kendoGrid({
            ...
            columns: [
               {field: "reason_code",title: "Reason Code",width: 107},
               {field: "reason_desc",title: "Description"}
           ],
           change: function(e){
           		currRow = this;
        ...
        insertGridTitle("#rowSelection","Reason Listing");
	    			...
	    			case "saveButt":
						...
						if (cMode == "add"){
							dataSource.add({PROGRESS_RECID: 0,
											reason_code: $("#txt1").val(),
											reason_desc: $("#textarea").val(),
					        ...
					        $.post(crudService + "manage/reason",{PROGRESS_RECID: dataItem.PROGRESS_RECID,reason_code: $("#txt1").val(), reason_desc: $("#textarea").val(), flg_status: ($("#rad1").is(":checked") ? 1 : 0)},
					       	    function(data){
									$("#rowSelection").data("kendoGrid").setDataSource(dataSource);
									$("#rowSelection").data("kendoGrid").dataSource.page($("#rowSelection").data("kendoGrid").dataSource.page());
									$("#rowSelection").data("kendoGrid").dataSource.read();
