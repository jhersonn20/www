...
<div id="main-wrapper" >
	<div class="wrap-button demo-section" style="width: 39.5%;min-height: 530px;position: fixed;top: 120px;left: 400px;">
		...
		<div><br /></div>
		<div >
		<input type="text" name="txt2" id="txt2" style="width: 155px;" />
		</div>
		<div><br /></div>
		<div>End Date:</div>
		<div><br /></div>
		<div >
		<input type="text" name="txt3" id="txt3" style="width: 155px;" />
		</div>
		<div>
		<label class="title" for="sel1" >Tespack No:</label><select name="sel1" id="sel1" style="width: 300px" ></select>
		</div>
		<div><br /></div>
		<div>
			<label class="title" for="sel3">Drawing No:</label><select name="sel3" id="sel3" style="width: 300px;"></select>
		</div>
		<div><br /></div>
	
		<div ><h2>Report Option</h2> </div>	
		<div><br /></div>
		<div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
			...
			var testPacker = $("#sel1").kendoMultiSelect({
            filter: "contains",
            placeholder: "Select Testpack...",
            dataTextField: "testpack_no",
            dataValueField: "testpack_no",
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/testpack_dd",
                    	type: "GET"
				    }
                },
        ...
        var drawingNo = $("#sel3").kendoMultiSelect({
            ...
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/dd_drawing",
                    	type: "GET"
				    }
                },
                //serverFiltering: true,
		    		...
		    		switch(this.id){
		    			case "printButt":
							...
							$("#window").data("kendoWindow").refresh({
							    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/Fabricated Pipe Support Requested Issued" + "&cdraw=" + drawingNo.value()  + "&testPack=" + testPacker.value() + "&fiDtFrom=" + kendo.toString(start.value(),"yyyy-MM-dd") + "&fiDtTo=" + kendo.toString(end.value(),"yyyy-MM-dd")  + "&iOption=" + $('input[name=option]:checked').index('input[name=option]'),
