...
<div id="main-wrapper" style="min-height: 300px">
	<div class="wrap-button demo-section" style="width: 40%;min-height:500px;position: absolute;top: 120px;left: 400px;">
		...
		<div><br /></div> -->
		<div>
		<label class="title" for="sel1" >Priority No:</label><select name="sel1" id="sel1" style="width: 300px" ></select>
		</div>
		<div><br /></div>
		<div>
			...
			$(".wrap-button .buttonRight button").bind({
		    	click: function(e){
		    		switch(this.id){
		    			 ...
		    			 case "printButt":
		    			 //alert(areaNo.value() + " " + plantNo.value()  + " " + priorityCode.value());
	    				if (confirm("Do you really want to print this item?")){	    					
							...
							$("#window").data("kendoWindow").refresh({
							    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/Material per Priority Register&carea=" + areaNo.value() + "&cplants=" + plantNo.value() + "&cpriior=" + priorityCode.value() + "&ioption=" + $('input[name=option]:checked').index('input[name=option]'),
