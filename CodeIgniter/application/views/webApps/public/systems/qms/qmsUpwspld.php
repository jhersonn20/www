	...
	$(document).ready(function(){
        ...
        var grid = $("#rowSelection").kendoGrid({
            ...
            columns: [                      
		       ...
		       {field: "spool_no1",width: 90,title: "Spool No."},           
		       {field: "spool_no",width: 96,title: "Spool Mark"},           
		       {field: "spool_desc",width: 134,title: "Stock Description"},           
		       {field: "jwrr_no",width: 90,title: "JWRR No."},                  
		       {field: "recvd_date",width: 275,title: "[MM/DD/YYY] - WHSE Spool Recvd. Date"},
		       {field: "jmif_no",width: 90,title: "JMIF No."},                  
		       {field: "issued_date",width: 277,title: "[MM/DD/YYY] - WHSE Spool Issued Date"},
				...
				switch(this.id){
					...
					default:
						...
						$.each(dataSource.data(),function(index,value){
							...
							row['pip_iso_no1'] = row['pip_iso_no'] + "-" + (row['sheet_no'].length == 1 ? ("0" + row['sheet_no']) : row['sheet_no']);
							row['loguser'] = $("#hidden_user").val();
							row['logupdate'] = "Upload Warehouse Spool Data";
							...
							$.post(crudService + "manage/whseMto",postInfo,
								function(data){
									if ((ds_total - 1) == index)
