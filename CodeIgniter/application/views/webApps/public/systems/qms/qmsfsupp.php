		...
		<div class="wrap-form demo-section apply8" style="width: 37.1%;float:right;height: auto;display: block;">
			<fieldset>
				...
				<ul class="formLeft_qms" style="width: 100%;">
					<li>
						<label class="title" for="txt1" style="width: 130px;">Supplier:</label><input type="text" required name="txt1" id="txt1" class="k-textbox" style="width: 61%;" />
					</li>
					<li>
		...
		<div id="jmifDtlHead" style="margin-bottom: 5px;">
		    <div class="wrap-grid demo-section" style="width: 61%;margin-left: 0;height: auto;">
				...
				<fieldset style="margin-top: 5px;">
					<ul style="float: left;">
						<li>
							<label class="title" for="txt2" style="width: auto;">Log User:</label><input type="text" name="txt2" id="txt2" class="k-textbox k-state-disabled" disabled style="width: 85px;" />
							<label class="title" for="txt2" style="width: auto;">Log Date:</label><input type="text" name="txt3" id="txt3" class="k-textbox k-state-disabled" disabled style="width: 85px;" />
							<label class="title" for="txt2" style="width: auto;">Log Update:</label><input type="text" name="txt4" id="txt4" class="k-textbox k-state-disabled" disabled style="width: 200px;" />
						</li>
					</ul>
				</fieldset>
		    </div>
...
<script type="text/javascript">
	function grid_change(e){
	    ...
	    for (var i = 0; i < selectedRows.length; i++) {
	      dataItem = e.dataItem(selectedRows[i]);
		    $("#txt1").val(dataItem.supp_code);
		    $("#textarea").val(dataItem.supp_desc);
	...
	$(document).ready(function(){
        ...
        var dataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: crudService + "directCall/supplierRef",
                    contentType: "application/json",
                    type: "GET"
                },
                create: {
                    url: crudService + "manage/supplierRef",
                    type: "POST",
                ...
                destroy: {
                    url: crudService + "remove/supplierRef",
                    type: "POST"
                },
			    parameterMap: function(data, type) {
			      if (type == "read") {
				    ...
				    fieldSort = ($(data.sort).length ? data.sort[0].field : "supp_code");
				    query = filterFArr;
        ...
        var grid = $("#rowSelection").kendoGrid({
            ...
            columns: [
               {field: "supp_code",title: "Supplier",width: 102},
               {field: "supp_desc",title: "Description"}
           ],
           change: function(e){
           		currRow = this;
	    		...
	    		switch(this.id){
	    			case "saveButt":
						...
						if (cMode == "add"){
					        ...
					        $.post(crudService + "manage/supplierRef",{PROGRESS_RECID: dataItem.PROGRESS_RECID,supp_code: $("#txt1").val(), supp_desc: $("#textarea").val(), flg_status: ($("#rad1").is(":checked") ? 1 : 0), log_user: $("#hidden_user").val()},
					       	    function(data){
									$("#rowSelection").data("kendoGrid").setDataSource(dataSource);
									$("#rowSelection").data("kendoGrid").dataSource.page($("#rowSelection").data("kendoGrid").dataSource.page());
									$("#rowSelection").data("kendoGrid").dataSource.read();
