	...
	$(document).ready(function(){
        ...
        var grid = $("#rowSelection").kendoGrid({
            ...
            columns: [                      
		       ...
		       {field: "sheet_no",width: 90,title: "Sheet No."},           
		       {field: "item_code",width: 90,title: "Item Code"},                      
		       {field: "size",width: 90,title: "Size"},
		       {field: "spool_no",width: 90,title: "Spool No"},                    
		       {field: "spool_type",width: 93,title: "Stock Type"},  
		       {field: "spool_category",width: 124,title: "Stock Category"},           
		       {field: "mto_qty",width: 90,title: "Qty."},                             
		       {field: "matl_desc",width: 152,title: "Material Description"},                             
		       {field: "jmif_issued_qty",width: 127,title: "JMIF Issued Qty"},
		       {field: "jmif_issued_date",width: 226,title: "[MM/DD/YYY] - JMIF Issued Date"},
		       {field: "jmif_no",width: 90,title: "JMIF No."},
				...
				switch(this.id){
					...
					default:
						...
						$.each(dataSource.data(),function(index,value){
							...
							row['pip_iso_no1'] = row['pip_iso_no'] + "-" + (length(row['sheet_no']) == 1 ? ("0" + row['sheet_no']) : row['pip_iso_no1']);
							row['loguser'] = $("#hidden_user").val();
							row['PROGRESS_RECID'] = 0;
							row['index'] = index;
							row['logupdate'] = "Upload Warehouse Materials Data";
							row['file_origin'] = fileName;
							postInfo = JSON.stringify(row);
							postInfo = eval("(" + postInfo + ")");

							$.get(crudService + "manage/whseMatMto",postInfo,
								function(data){
									if ((ds_total - 1) == index)
