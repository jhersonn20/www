	...
	$(document).ready(function(){
        ...
        var grid = $("#uploadBrowse").kendoGrid({
            ...
            columns: [
	            ...
	            {field: "fogfitup_date",width: 92,title: "FOG Fitup"},              
				{field: "fog_installed",width: 114,title: "FOG Installed"},       
				{field: "fog_remarks",title: "FOG Remarks"}
           ],
           change: function(e){
           		currRow = this;
			...
			$.get("/assets/uploads/" + e.files[0].name, function(data) {
				...
				$.each(data.rows, function(index,value){
					...
					$.get(crudService + "directCall/verifyPipe",{rev_no: this.rev_no, area_no: this.area_no, drawing_no: this.drawing_no, spool_no: this.spool_no, sheet_no: this.sheet_no, mat_tag: this.mat_tag},
						function(callback){
							if (callback == "false"){
								rows[index]['err_remarks'] = "Pipe Support Material Not Found";
								errCounter++;
							}
			...
			click: function(){
				switch(this.id){
					...
					default:
						...
						$.post(crudService + "directCall/psFogMto",{rows: $("#objStr").val()},
							function(data){
