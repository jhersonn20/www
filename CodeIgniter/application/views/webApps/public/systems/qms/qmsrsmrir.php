...
<div id="main-wrapper">
	<div class="wrap-button demo-section" style="width: 39.5%;min-height: 460px;position: fixed;top: 120px;left: 400px;">
		...
		<div><br /></div>
		<div>
			<label class="title" for="sel4">Plant No:</label><select name="sel4" id="sel4" style="width: 300px;"></select>
		</div>
		<div><br /></div>
		<div>
		...
		<div><br /></div>
		<div>
			<label class="title" for="sel3">Area loc:</label><select name="sel3" id="sel3" style="width: 300px;"></select>
		</div>
		<div><br /></div>
		<div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
			...
			var priorityCode = $("#sel1").kendoMultiSelect({
            filter: "contains",
            placeholder: "Select Priority Code...",
            dataTextField: "priority_code",
            dataValueField: "priority_code",
            dataSource: {
                transport: {
                    read: {
                        url: crudService + "directCall/priority_ref",
                    	type: "GET"
				    }
                },
        ...
        var plantNo = $("#sel4").kendoMultiSelect({
            filter: "contains",
         ...
         var start = $("#txt1").kendoDatePicker({
			...
			$(".wrap-button .buttonRight button").bind({
		    	click: function(e){
		    		switch(this.id){
		    			case "printButt":
		    			//alert(areaNo.value() +  $('input[name=option]:checked').val() +  areaLoc.value() +  plantNo.value() +  priorityCode.value() +  kendo.toString(start.value(),"yyyy-MM-dd"));
	    				if (confirm("Do you really want to print this item?")){	    					
							...
							$("#window").data("kendoWindow").refresh({
							    url: "http://" + $(location).attr('hostname') + ":8080/JavaBridge/executeReport.php?xmlFile=qms/SPOOL MAT REC ISSUANCE&cArea=" + areaNo.value() + "&cBore=" + $('input[name=option]:checked').val() + "&cLoc=" + areaLoc.value() + "&cPlant=" + plantNo.value() + "&cPrior=" + priorityCode.value() + "&dtAsof=" + kendo.toString(start.value(),"yyyy-MM-dd"),
