...
<script type="text/javascript">
	$(document).ready(function(){ 
		var crudService = crudServiceBaseUrl + "index/",
		    filterFArr_user = [], filterOArr_user = [], filterVArr_user = [], user_di = "", log_user = '<?php echo $_POST['log_user']; ?>';
	    ...
	    var dept_CB = function(container, options) {
	        $('<input ' + (options.model.client_short == "ARCC" ? 'required' : '') + ' data-text-field="option" data-value-field="value" id="department" data-bind="value:' + options.field + '"/>')
	            .appendTo(container)
	            .kendoComboBox({
				...
				switch(this.id){
					case "chgButt":
						...
						$("#subWindow").data("kendoWindow").refresh({
							url: "/index/direct_to/tmpl_changePassword",
							type: "POST",
					...
					default:
		    			if (this.id == "addButt"){
							...
							$("#subWindow").data("kendoWindow").refresh({
								url: "/index/direct_to/portRuser_add",
								type: "POST",
