<div class="signup">
	<h1> Sign Up </h1> | <a href="#"> User Reference</a> <?php echo ($department == "MIS") ? "| <a href='#'> File Reference</a> | <a href='#'> Assign Client</a>" : ""; ?> <!--| <a href="#"> Preferences </a>-->
	...
	<ul>			
		...
		<li> <label class="title" for="dept" style="width: 36%;">Department:</label><input type="text" name="dept" id="dept" style="width: 53%" />
			 <button type="button" name="dept_ref" id="dept_ref" class="k-button">...</button>
		</li>
		...
		<li> <label class="title" for="email_add" style="width: 36%;">Email Address:</label><input required  type="email" name="email_add" id="email_add"   class="k-textbox" style="width: 62%;" /> </li>
...
<!-- 		<li> <label class="title" for="conf_pword" style="width: 36%;">Confirm Password:</label><input required type="password" name="conf_pword" id="conf_pword" class="k-textbox" style="width: 62%;" /> </li> -->
	...
	<hr style="margin: 3px 0;" />
	<button name="connect" id="connect" class="k-button" style="float: left;">Add Recipient</button>
	<button name="create" id="create" class="k-button" style="float: right;">Create</button>
</div>
<div class="latest">
	<h1> Latest / Un-Open Uploads </h1>
	<!-- <input type="text" id="inboxLatestFil"  name="inboxLatestFil" value="ALL" /> -->
	<hr style="margin: 3px 0;" />
    ...
    <div class="module_footer">
		<hr style="margin: 3px 0;" />
		<button name="connect" id="connect2" class="k-button" style="float: left;display: none;">Add Recipient</button>
    ...
    <div id="tabstrip" class="login">
	    <ul>
		    ...
		    <li class="login">
				...
				<!-- <input type="text" id="inboxFil"  name="inboxfilter" value="ALL" style="width: 120px; min-height: 27px;" /> -->
		</ul>
		<div class="login">
...
<script type="text/javascript">
	$(document).ready(function(){   
		var crudService = '/codeigniter/index.php/portal/' + "index/", 
			...
			currRow = "", currRow_upl = "", currRow_dl = "", upload_count = 0, log_user = '<?php echo $user_id; ?>', file_types = [], user_pk = "<?php echo $user_pk; ?>", client_desc = '';
		
		
		$(".signup").each(function(index,value){	
			if (dept != 'MIS'){
				if (dept != 'MKTG'){
				// $("#dept").css('width',223);
				// $("#dept_ref").hide();
				$(".signup").css({"display": "none"});				$(".latest").css({"width": '984px'});
				$("#connect2").css({"display": "block"});
				}
			}
		});
            ...
            schema: {
               ...
               model: {
                    ...
                    fields: {
				        ...
				        remarks: {type: "string", editable: false},
				        log_created: {type: "string", editable: false},
				        log_date: {type: "date", editable: false},
                    }
               },
               total: function(response) {
				   	return (($(response.rows[0]).length > 0) ? response.rows[0].total : 0); //$(response.rows).length;				   	
...
// 		
       ...
       var inboxFilter = $("#inboxFil").kendoComboBox({
	        highlightFirst: false,
            filter: "contains",
            placeholder: "Inbox Filter",
		    dataTextField: "option",
		    dataValueField: "value",
		    enable: true,
		    dataSource: [
		        { option: "Show All", value: "ALL" },
		        { option: "30 Days", value: "30" },
		        { option: "25 Days", value: "25" },
		        { option: "20 Days", value: "20" },
		        { option: "15 Days", value: "15" },
		        { option: "10 Days", value: "10" },
        ...
        var inboxLatest = $("#inboxLatestFil").kendoComboBox({
	        highlightFirst: false,
	        filter: "contains",
            placeholder: "Inbox Filter",
		    dataTextField: "option",
        ...
        var dept_cb = $("#dept").kendoComboBox({
		    ...
		    dataSource: {
	                transport: {
	                    read: crudService + "directCall/get_dept_dd",
	            		contentType: "application/json",
	            	},
	                schema: {
						data: function(data){
			...
			change: function(e){
            	...
            	// if (this.value() == 'MKTG' || this.value() == 'MIS')
		...
		$(".signup a").eq(0).click(function(e){
			...
			$("#window").data("kendoWindow").refresh({
				...
				type: "POST",
				data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 4), log_user: log_user, follows: follows,dept: dept}
			});
        	$("#window").data("kendoWindow").center().open();
		});
		...
		$(".signup a").eq(2).click(function(e){
			...
			$("#window").data("kendoWindow").refresh({
				url: "<?php echo '/codeigniter/index.php/portal'; ?>/index/direct_to/portRdclient",
				type: "POST",
				data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 4), log_user: log_user, follows: follows,dept: dept}
			});
        	$("#window").data("kendoWindow").center().open();
				...
				switch(this.id){
					...
					case "client_ref":
						...
						$("#window").data("kendoWindow").refresh({
							...
							type: "POST",
							data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 5), log_user: log_user, department: dept}
						});
			        	$("#window").data("kendoWindow").center().open();
						break;
					case "gen_pw":
					...
					case "connect2":
						...
						$("#window").data("kendoWindow").refresh({ 
							...
							type: "POST",
							data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 4), log_user: log_user, follows: follows, user_pk: user_pk, department: dept}
						});
			        	$("#window").data("kendoWindow").center().open();
						break;
					case "dept_ref":
						...
						$("#window").data("kendoWindow").refresh({
							url: "<?php echo '/codeigniter/index.php/portal'; ?>/index/direct_to/portRdept",
							type: "POST",
							data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 5), log_user: log_user, department: dept}
						});
			        	$("#window").data("kendoWindow").center().open();
						break;	
					default:
						if (client_cb.value() == 15){
							client_desc = dept_cb.value();
						}else {
							client_desc = client_cb.text();
						}
				        ...
				        $.post('/codeigniter/index.php/portal/index/' + "manage/user",{id: 0, client_id: client_cb.value(), first_name: $("#first_name").val(), last_name: $("#last_name").val(), 
				        									...
				        									expiry: kendo.toString(expiry.value(),"yyyy-MM-dd HH:mm:ss"), creator:((dept == 'MKTG') ? 'MKTG' : ''),creator_id:((dept == 'MKTG') ? log_user : ''), log_user: log_user, department: client_desc, follows: user_pk},
				       	    function(data){
				       	    	if (data != '1')
									showNotif('Warning',data,'warning');
								else {
