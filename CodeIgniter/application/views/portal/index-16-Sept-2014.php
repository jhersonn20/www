...
<div class="signup">
	<h1> Sign Up </h1> | <a href="#"> User Reference</a> | <a href="#"> File Reference</a> <!--| <a href="#"> Preferences </a>-->
...
<script type="text/javascript">
	$(document).ready(function(){   
		var crudService = crudServiceBaseUrl + "index/", 
			...
			currRow = "", currRow_upl = "", currRow_dl = "", upload_count = 0, log_user = '<?php echo $user_id; ?>', file_types = "";
        ...
        var onSelect = function(e){
			...
			if (client == 15){
				...
				$("#window").data("kendoWindow").refresh({
					url: "/index/direct_to/portRclient_list",
					type: "POST",
...
// 		
		...
		$("#files").kendoUpload({
			...
			upload: function(e){
		    	...
		    	e.data = {client_ids: client_ids, remarks: remarks_txt, user_ids: user_ids, send_this: (upload_count == 0)}; //, email_adds: email_adds
			},
			remove: function(e){
			...
			click: function(){
				switch(this.id){
					case "open":
				        ...
				        $("#window").data("kendoWindow").center().open();
				        $.post("/index/manage/files",{id: dataItem.id, log_user: log_user, date_open: kendo.toString(new Date(),"yyyy-MM-dd")},
				        	function(data){
				       	    	if (data != '1')
					...
					case "open2":
				        ...
				        $("#window").data("kendoWindow").center().open();
				        $.post("/index/manage/files",{id: dl_di.id, log_user: log_user, date_open: kendo.toString(new Date(),"yyyy-MM-dd")},
				        	function(data){
				       	    	if (data != '1')
									showNotif('Warning',data,'warning');
				        	});
					...
					case "client_ref":
						...
						$("#window").data("kendoWindow").refresh({
							url: "/index/direct_to/portRclient",
							type: "POST",
							data: {dl_ds_length: (dataSource._data.length + upl_ds._data.length + dl_ds._data.length + react_ds._data.length + 5)}
						});
			        	$("#window").data("kendoWindow").center().open();
					...
					default:
				        ...
				        $.post(crudService + "manage/user",{id: 0, client_id: client_cb.value(), first_name: $("#first_name").val(), last_name: $("#last_name").val(), 
				        									...
				        									expiry: kendo.toString(expiry.value(),"yyyy-MM-dd HH:mm:ss"), log_user: log_user, department: dept_cb.value()},
				       	    function(data){
				       	    	if (data != '1')
									showNotif('Warning',data,'warning');
								else {
