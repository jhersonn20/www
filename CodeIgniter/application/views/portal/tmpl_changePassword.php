<!--<script type="text/x-kendo-template" id="changePassword">-->
	...
	<hr style="margin-bottom: 5px;" />
		<div class="buttonLeft" style="float: left;">
			<button type="button" name="gen_pw" id="gen_pw" class="k-button">Generate Password</button>
		</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#subWindow input").keypress(function(){
					$('#newPass, #confPass').attr('type','password');
			});
			
			$(".buttonLeft button").bind({
		    	click: function(e){
		    		switch(this.id){
		    			case "gen_pw":
		    				 $("#subWindow input").unbind('keyup');	
		    				 $("#newPass, #confPass").val(Math.random().toString(36).slice(5));
		    				 $('#newPass, #confPass').attr('type','text');
		    				 $("#subWindow input").unbind('keyup');
		    				 $("#applyButt").prop('disabled',false).removeClass("k-state-disabled");
			...
			$("#applyButt").click(function(){
				var email_add = "<?php echo $_POST['email_add']; ?>";
				//alert($(location).attr("hostname") + " " + (v_hostname + "/index/manage/user"));
				$.post(v_hostname + "/index/manage/user",{user_id: $("#user").val(), currPass: $("#currPass").val(), password: $("#newPass").val(), id: <?php echo $_POST['id']; ?>, log_user: log_user, email_add: email_add},
				function(data){
					if (data != 1){
