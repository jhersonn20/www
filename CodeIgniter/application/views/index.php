...
<body>
    ...
    ?>
    <ul id="options">
        <li><a href="<?php echo base_url(); ?>example/create_table">Make Database Table</a>: Requires you to setup the database paramaters in "/system/application/config/database.php"</li>
        <li><a href="<?php echo base_url(); ?>example/create">Create New Entry</a>: This will attempt to generate a new entry, but will check for duplicates.  So if it comes back and says a duplicate is found. Click it agian.</li>
    ...
    if (empty($table_list))
    {
        ...
        foreach($table_list as $value)
        {
            echo '
        <tr>
            <td><a href="'. base_url() .'example/delete/'.$value['blog_id'].'">delete</a> | <a href="'. base_url() .'example/update/'.$value['blog_id'].'">edit</a> | <a href="'. base_url() .'example/retrieve/'.$value['blog_id'].'">view</a></td>
            <td>'.$value['blog_id'].'</td>
