...
if (!defined('RANDOMNO'))
	...
	class Index extends CI_Controller{		
		...
		public function __construct(){
			parent::__construct();
            $this->load->helper(array('form','url','file','date','csv_helper','email'));
            $this->load->library(array('zip','portal/email','session','form_validation','portal/downloader'));
		}
		...
		public function index(){
	        ...
	        if ($data['is_logged_in']){
	        	...
	        	$data['follows'] = $this->session->userdata('follows');
			...
			switch($item){
				...
				case "user_assign":
					// $fieldVal = ($this->session->userdata('department') == "MIS" ? "1" : ("t.log_created like '" . $this->session->userdata('user_id') . "%'"));
 					// $fieldVal = "t.log_created like '" . $this->session->userdata('user_id') . "%' and t.user_id != '" . $this->session->userdata('user_id') . "'"; //improvements 					
					//$fieldVal = "find_in_set(t.id,'" . $_GET['follows'] . "') != 0 and t.user_id != '" . $this->session->userdata('user_id') . "'"; //improvements
					$fieldVal = "t.client_id != 15"; //t.creator = '' and 
					$this->load->model('portal/ruser_model');
					$this->read("ruser", "get_all", $fieldVal, $dbArr);
					break;	
				case "user":
				...
				case "modified_connect":
					if (($this->session->userdata('department') == "MIS")){
						...
						if ($_GET['type'] == "1") 					
							...
							$fieldVal = "find_in_set(t.id,'" . $_GET['follows'] . "') = 0 and t.user_id != '" . $this->session->userdata('user_id') . "'"; //improvements . " and t.department = '" . $this->session->userdata('department') . "'" -- . " and t.creator = '" . $this->session->userdata('department') . "'"
						$this->load->model('portal/ruser_model');
...
//            
		...
		public function direct_dl(){
			...
			$dl_speed = $this->file_ref('get');
			$dl_speed = explode(";", $dl_speed);
			// $file_path = 'e:/portal/documents/15/jarir-shopping-guidess.zip';
			...
			// // // echo $dl_speed[1] . "<br />";
			// // $delay = (256 / intval($dl_speed[1])) * 1000;
			// $file_path = 'e:/portal/documents/' . $_GET['client_id'] . "/" . addslashes($_GET['name']);
			// // echo $delay . "<br />";
			// // echo $file_path . "<br />";
			// $finfo = finfo_open(); 
...
// 			
			...
			// readfile($file_path); //$value['client_id'] . "/" . $value['name']);
		    $this->load->model("portal/audit_model");
			...
			// // while (!feof($target_file)) {
				// // // echo fread($target_file, 256);
				// // print fread($target_file, 256); //round(intval($dl_speed[1]) * 1024));
				// // flush();
				// // usleep($delay);
				// // // sleep(1);
			// // }
		...
		public function manage($item = "",$postInfo = array()){			
			...
			switch($item){
				...
				case "file_users":
					...
					break;
				case "user":
					if (valid_email("{$_POST['email_add']}")){
						 $this->load->model('portal/ruser_model');
				...
				case "department":
					$this->load->model('portal/department_model');
					$this->callback($this->department_model->set($postInfo));
					break;	
				default:
					break;
			}
		...
		public function remove($item = ""){
			...
			switch($item){
				...
				case "department":
					$this->load->model('portal/department_model');
					$this->callback($this->department_model->remove());
					break;	
				default:
	            ...
	            if (!empty($data['users_item'])){
	            	if (new DateTime($data['users_item']['expiry']) >= new DateTime("now")){
						...
						$newData = array("user_id"=>$data['users_item']['user_id'],
		                				 ...
		                				 "follows"=>$data['users_item']['follows']);
