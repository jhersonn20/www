/*
 ...
 Date: 2012-10-18
*/
(function(a){a.jgrid=a.jgrid||{};a.extend(a.jgrid,{version:"4.4.2",htmlDecode:function(a){return a&&("&nbsp;"==a||"&#160;"==a||1===a.length&&160===a.charCodeAt(0))?"":!a?a:(""+a).replace(/&gt;/g,">").replace(/&lt;/g,"<").replace(/&quot;/g,'"').replace(/&amp;/g,"&")},htmlEncode:function(a){return!a?a:(""+a).replace(/&/g,"&amp;").replace(/\"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")},format:function(d){var e=a.makeArray(arguments).slice(1);void 0===d&&(d="");return d.replace(/\{(\d+)\}/g,
...
1),b.push(d)):0<l.overlay?l.closeoverlay&&j.w.jqmAddClose(s):s=c;j.o=s?s.addClass(l.overlayClass).prependTo("body"):c;if(f)try{if(a("html,body").css({height:"100%",width:"100%"}),s){var s=s.css({position:"absolute"})[0],r;for(r in{Top:1,Left:1})s.style.setExpression(r.toLowerCase(),"(_=(document.documentElement.scroll"+r+" || document.body.scroll"+r+"))+'px'")}}catch(t){}l.ajax?(p=l.target||j.w,s=l.ajax,p="string"==typeof p?a(p,j.w):a(p),s="@"==s.substr(0,1)?a(g).attr(s.substring(1)):s,p.html(l.ajaxText).load(s,
...
this;if(f.grid){var c=a(f).jqGrid("getRowData",d);if(c)for(var h in c)a("[name="+a.jgrid.jqID(h)+"]",b).is("input:radio")||a("[name="+a.jgrid.jqID(h)+"]",b).is("input:checkbox")?a("[name="+a.jgrid.jqID(h)+"]",b).each(function(){if(a(this).val()==c[h])a(this)[f.p.useProp?"prop":"attr"]("checked",!0);else a(this)[f.p.useProp?"prop":"attr"]("checked",!1)}):a("[name="+a.jgrid.jqID(h)+"]",b).val(c[h])}})},FormToGrid:function(d,b,f,c){return this.each(function(){if(this.grid){f||(f="set");c||(c="first");
var h=a(b).serializeArray(),g={};a.each(h,function(a,b){g[b.name]=b.value});"add"==f?a(this).jqGrid("addRowData",d,g,c):"set"==f&&a(this).jqGrid("setRowData",d,g)}})}})})(jQuery);
