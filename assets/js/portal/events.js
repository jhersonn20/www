var crudServiceBaseUrl = "",
    ...
    skip_this_field = [],
    client_ids = "", remarks_txt = "", email_adds = "", user_ids = "";
...
$(document).ready(function(){
    ...
    stripTags = function(text) { 
    	return this.replaceWith( text.replace(/<\/?[^>]+>/gi, '') ); 
    };
    
	notif = function(type,title,text){
		if ($(".ui-pnotify").is(":visible"))
			if (stripTags($(".ui-pnotify-text").html()) == stripTags(text))
				return true;
    ...
    $("#window").kendoWindow({ 
        ...
        scrollable: true,
        activate: function(){
        	this.wrapper.css({top: (($("#arccWrap").height() > this.wrapper.height()) ? (parseInt($("#arccWrap").height()) - parseInt(this.wrapper.height())) / 2 : 0)});
